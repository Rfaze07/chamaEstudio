...
exports.getLista = async (req, res) => {
    try {
        res.render('tipos_materias_primas/views/index', {
            pagename: "Tipos de Materias Primas",
...
exports.postAlta = async (req, res) => {
    try {
        ...
        await eventos.insertarEvento({ usuario: req.session.user.id, tabla: "Tipos de Materias Primas", acc: "a", registro: resInsert.insertId })
...
exports.postModificar = async (req, res) => {
    try {
        ...
        await eventos.insertarEvento({ usuario: req.session.user.id, tabla: "Tipos de Materias Primas", acc: "m", registro: req.body.id })
...
exports.postEliminar = async (req, res) => {
    try {
        ...
        //Validar que no tenga materias primas asociadas , a futuro

        let result = await model.delete(req.body.id)
        if(result.affectedRows == 0) return res.json({ status: false, icon: "error", title: "Error", text: "Hubo un error al procesar la solicitud" })
        await eventos.insertarEvento({ usuario: req.session.user.id, tabla: "Tipos de Materias Primas", acc: "b", registro: req.body.id })            
        return res.json({ status: true, icon: "success", title: "Éxito", text: "Solicitud procesada correctamente" })
    } catch (error) {
        console.log(error)
        return res.json({ status: false, icon: "error", title: "Error", text: "Hubo un error al procesar la solicitud" })
