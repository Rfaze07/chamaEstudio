...
exports.getLista = async (req, res) => {
    try {
        res.render('materias_primas/views/index', {
            pagename: "Materias Primas",
...
exports.postAlta = async (req, res) => {
    try {
        ...
        await eventos.insertarEvento({ usuario: req.session.user.id, tabla: "Materias Primas", acc: "a", registro: resInsert.insertId })
...
exports.postModificar = async (req, res) => {
    try {
        ...
        await eventos.insertarEvento({ usuario: req.session.user.id, tabla: "Materias Primas", acc: "m", registro: req.body.id })
...
exports.postEliminar = async (req, res) => {
    try {
        ...
        if(result.affectedRows == 0) return res.json({ status: false, icon: "error", title: "Error", text: "Hubo un error al procesar la solicitud" })
        await eventos.insertarEvento({ usuario: req.session.user.id, tabla: "Materias Primas", acc: "b", registro: req.body.id })            
        return res.json({ status: true, icon: "success", title: "Éxito", text: "Solicitud procesada correctamente" })
    } catch (error) {
        console.log(error)
        return res.json({ status: false, icon: "error", title: "Error", text: "Hubo un error al procesar la solicitud" })
...
const ValidarCampos = o => {
    return new Promise((resolve, reject) => {
        ...
        if(String(o.id_tipo_materia_prima_fk).trim().length == 0) return res.json({ status: false, icon:'error', title: 'Error', text: 'Debe seleccionar el tipo de materia prima' })
        if(String(o.id_unmed_fk).trim().length == 0) return res.json({ status: false, icon:'error', title: 'Error', text: 'Debe seleccionar la unidad de medida' })
